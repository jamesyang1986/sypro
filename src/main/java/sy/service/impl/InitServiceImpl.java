...
public class InitServiceImpl implements InitServiceI {
    ...
    private void initResource() {
        ...
        userCreateDatetimeChart
                .setUrl("/chartController/userCreateDatetimeChart");
    ...
    private void initRole() {
        ...
        zyAdmin.getTresources()
                .addAll(resourceDao
                        .find("from Tresource t where t.tresource.id in ('zygl') or t.id in ('zygl')"));
        ...
        jsAdmin.getTresources()
                .addAll(resourceDao
                        .find("from Tresource t where t.tresource.id in ('jsgl') or t.id in ('jsgl')"));
        ...
        yhAdmin.getTresources()
                .addAll(resourceDao
                        .find("from Tresource t where t.tresource.id in ('yhgl') or t.id in ('yhgl')"));
        ...
        sjyAdmin.setSeq(4);
        sjyAdmin.getTresources()
                .addAll(resourceDao
                        .find("from Tresource t where t.tresource.id in ('sjygl') or t.id in ('sjygl')"));
        ...
        bugAdmin.setSeq(5);
        bugAdmin.getTresources()
                .addAll(resourceDao
                        .find("from Tresource t where t.tresource.id in ('buggl') or t.id in ('buggl')"));
    ...
    private void initUser() {
        List<Tuser> l = userDao
                .find("from Tuser t where t.name in ('孙宇','admin1','admin2','admin3','admin4','admin5','guest')");
        if (l != null && l.size() > 0) {
            for (Tuser user : l) {
                userDao.delete(user);
            }
        ...
        admin1.getTroles().addAll(
                roleDao.find("from Trole t where t.id = 'zyAdmin'"));// 给用户赋予资源管理员角色
        ...
        admin2.getTroles().addAll(
                roleDao.find("from Trole t where t.id = 'jsAdmin'"));// 给用户赋予角色管理员角色
        ...
        admin3.getTroles().addAll(
                roleDao.find("from Trole t where t.id = 'yhAdmin'"));// 给用户赋予用户管理员角色
        ...
        admin4.getTroles().addAll(
                roleDao.find("from Trole t where t.id = 'sjyAdmin'"));// 给用户赋予数据源管理员角色
        ...
        admin5.getTroles().addAll(
                roleDao.find("from Trole t where t.id = 'bugAdmin'"));// 给用户赋予BUG管理员角色
        ...
        guest.getTroles().addAll(
                roleDao.find("from Trole t where t.id = 'guest'"));// 给用户赋予Guest角色
