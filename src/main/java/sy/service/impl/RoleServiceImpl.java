...
public class RoleServiceImpl implements RoleServiceI {
    ...
    public Role get(String id) {
        ...
        params.put("id", id);
        Trole t = roleDao
                .get("select distinct t from Trole t left join fetch t.tresources resource where t.id = :id",
                        params);
        if (t != null) {
    ...
    public List<Role> treeGrid(SessionInfo sessionInfo) {
        ...
        if (sessionInfo != null) {
            ...
            tl = roleDao
                    .find("select distinct t from Trole t left join fetch t.tresources resource order by t.seq");
        }
    ...
    public List<Tree> tree(SessionInfo sessionInfo) {
        ...
        if (sessionInfo != null) {
            params.put("userId", sessionInfo.getId());// 查自己有权限的角色
            l = roleDao
                    .find("select distinct t from Trole t join fetch t.tusers user where user.id = :userId order by t.seq",
                            params);
        } else {
    ...
    public void grant(Role role) {
        ...
        if (role.getResourceIds() != null
            ...
            t.setTresources(new HashSet<Tresource>(resourceDao
