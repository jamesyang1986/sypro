...
public class BugServiceImpl implements BugServiceI {
    ...
    public DataGrid dataGrid(Bug bug, PageHelper ph) {
        ...
        List<Tbug> l = bugDao.find(hql + joinHql + whereHql(bug, params)
        ...
        dg.setRows(bl);
        dg.setTotal(bugDao
                .count("select count(*) " + hql + " left join t.tbugtype type "
                        + whereHql(bug, params), params));
        return dg;
    }
    ...
    public Bug get(String id) {
        ...
        params.put("id", id);
        Tbug t = bugDao.get(
                "from Tbug t join fetch t.tbugtype bugType where t.id = :id",
                params);
        Bug b = new Bug();
        BeanUtils.copyProperties(t, b, new String[] { "note" });
    ...
    public void edit(Bug bug) {
        ...
        if (t != null) {
            BeanUtils.copyProperties(bug, t, new String[] { "id", "note",
                    "createdatetime" });
            t.setTbugtype(bugTypeDao.getById(bug.getTypeId()));
